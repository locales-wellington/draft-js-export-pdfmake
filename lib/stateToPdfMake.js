...
var StateToPdfMake = function () {
    ...
    _createClass(StateToPdfMake, [{
        ...
        value: function _processBlock() {
            ...
            switch (block.getType()) {
                ...
                case _draftJsUtils.BLOCK_TYPE.ORDERED_LIST_ITEM:
                    {
                        var listItem = {
                            text: [].concat(_toConsumableArray(this._renderBlockContent(block)))
                        };
                ...
                case _draftJsUtils.BLOCK_TYPE.UNORDERED_LIST_ITEM:
                    {
                        ...
                        var currentDepth = this._findDepth(this.listUlAcc[this.listUlAcc.length - 1]);
                        // if currentLength = block.depth -> push normal
                        // if +1 push in array
                        // if +2 push in 2 arrays
                        // if -1 push in previous array
                        // c[current][][][];
                        var dif = block.depth - currentDepth;
                        console.log(block.depth, currentDepth, dif);
                        if (currentDepth === 0 && !dif) {
                            console.log(this.listUlAcc);
                            this.listUlAcc.ul.push(_listItem);
                        } else if (dif > 0) {
                            var tmpItem = _listItem;
                            while (dif) {
                                tmpItem = { ul: [tmpItem] };
                                dif--;
                            }
                            this._insertAtDepth(this.listUlAcc, tmpItem, currentDepth);
                        } else if (dif < 0) {
                            this._insertAtDepth(this.listUlAcc, _listItem, block.depth);
                        }
                    }
                    break;
        ...
        key: '_insertAtDepth',
        value: function _insertAtDepth(obj, item, depth) {
            if (!depth) {
                console.log('Obj', obj);
                obj.ul.push(item);
            } else {
        ...
        key: '_findDepth',
        value: function _findDepth(mightBeArray) {
            var currentDepth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;

            if (Array.isArray(mightBeArray)) {
                return this._findDepth(mightBeArray[mightBeArray.length - 1], currentDepth + 1);
            } else {
                return currentDepth;
            }
        }
